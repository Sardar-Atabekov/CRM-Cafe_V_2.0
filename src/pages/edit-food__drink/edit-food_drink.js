...
const AddIngredient = (props) => {
  ...
  useEffect(() => {
    getData(`meal/${props.match.params.id}/`)
      .then((res) => {
        setMealData(res);
        setLoading(true);
      })
  ...
  const postUserData = (e) => {
    ...
    // data.role = 5;
    putData(`meal-update/${props.match.params.id}`, data)
      .then((response) => {
        if (response.success) {
  ...
  return (
    <div className="wrapper">
      <Title>Изменение данных блюд</Title>
      {/* {loading ? ( */}
      <form className="input-blocks pt-4" onSubmit={postUserData}>
        ...
        <div className="form-group">
          ...
          <input
            ...
            name="amountPortion"
            className="form-control"
            defaultValue={mealData.amountPortion}
            id="amountPortion"
            required
          />
        </div>
